param([ValidateSet('solo','coop')][string]$Mode='solo')
$ErrorActionPreference='Stop'
$workspace=Split-Path $PSScriptRoot -Parent
$executable=Join-Path $workspace 'target/debug/chemgame.exe'
$roles=if($Mode -eq 'coop'){@('host','client')}else{@('solo')}
$processes=@()
$started=Get-Date
$run=$started.ToString('yyyyMMdd-HHmmss')
$previous=@{}
foreach($key in @('LOCALAPPDATA','APPDATA','BEVY_ASSET_ROOT','RUST_LOG','WGPU_BACKEND')){
    $previous[$key]=[Environment]::GetEnvironmentVariable($key,'Process')
}
try {
    foreach($role in $roles){
        $output=Join-Path $workspace "target/textbook-playtest/$role"
        New-Item -ItemType Directory -Force -Path "$output/$run/appdata","$output/$run/roaming" | Out-Null
        $env:LOCALAPPDATA="$output/$run/appdata"
        $env:APPDATA="$output/$run/roaming"
        $env:BEVY_ASSET_ROOT=$workspace
        $env:RUST_LOG='info,wgpu=warn,naga=warn'
        $env:WGPU_BACKEND='dx12'
        $launchArgs=if($role -eq 'client'){@('--join','127.0.0.1','--textbook-playtest')}
            elseif($role -eq 'host'){@('--host','--textbook-playtest')}
            else{@('--solo','--textbook-playtest')}
        $processes+=Start-Process -FilePath $executable -ArgumentList $launchArgs `
            -WorkingDirectory $workspace -WindowStyle Hidden -PassThru `
            -RedirectStandardOutput "$output/stdout.log" -RedirectStandardError "$output/stderr.log"
        if($role -eq 'host'){Start-Sleep -Seconds 8}
    }
} finally {
    foreach($key in $previous.Keys){[Environment]::SetEnvironmentVariable($key,$previous[$key],'Process')}
}
$deadline=[DateTime]::UtcNow.AddSeconds(100)
while(@($processes|Where-Object{-not $_.HasExited}).Count -gt 0){
    if([DateTime]::UtcNow -gt $deadline){$processes|Where-Object{-not $_.HasExited}|Stop-Process;throw 'Textbook test timed out; inspect logs.'}
    Start-Sleep -Seconds 1
}
foreach($role in $roles){
    $report=Get-Item -LiteralPath (Join-Path $workspace "target/textbook-playtest/$role/result.txt")
    if($report.LastWriteTime -lt $started){throw "$role did not write a fresh report."}
    $result=Get-Content -LiteralPath $report.FullName -Raw
    Write-Output $result
    if($result -notmatch '^PASS'){throw "$role failed."}
}
