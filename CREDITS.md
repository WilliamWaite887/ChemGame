...
| `stability_rumble.ogg` | `sound/ambience/earth_rumble/earth_rumble_distant2.ogg` | **CC0** | metrostock99; adapted by tgstation contributors | [Freesound 203281](https://freesound.org/people/metrostock99/sounds/203281/) ([pinned tgstation copy](https://github.com/tgstation/tgstation/blob/c16575c992d8f9adf666d7031f3f1ffc0193648a/sound/ambience/earth_rumble/earth_rumble_distant2.ogg)) |
| `stability_critical.ogg` | `sound/effects/explosion/explosioncreak1.ogg` | CC BY-SA 3.0 (tgstation project default — not listed in `sound/attributions.txt`, with no per-folder attribution manifest) | tgstation contributors | [Pinned tgstation file](https://github.com/tgstation/tgstation/blob/c16575c992d8f9adf666d7031f3f1ffc0193648a/sound/effects/explosion/explosioncreak1.ogg) |
...
These are unmodified copies from [Space Station 14](https://github.com/space-wizards/space-station-14). Most use revision [`f76827c45504f263fc540bc11a1ec7cde6c16977`](https://github.com/space-wizards/space-station-14/tree/f76827c45504f263fc540bc11a1ec7cde6c16977/Resources/Audio); the station-stability notice uses the newer pinned revision linked in its row. SS14's default for assets not named by an attribution manifest is CC BY-SA 3.0; the license-basis column distinguishes that default from explicit per-file metadata. CC0 files are included for traceability.

Every row was confirmed by **content, not filename**: each local file's git blob SHA-1 was compared against the SS14 blob at its pinned revision. The original 35 files and the later station-stability notice all matched exactly. That check also settled a near-miss — the `ambience/` files share SS14's filenames (`ambigen1.ogg` and friends) but none of the ten match SS14's blobs, so they are tgstation's and are credited in the tgstation table above, not here.

Most of these live in [assets/sounds/ss14/](assets/sounds/ss14/). Five announcement, alert, and station-condition stings sit at the root of `assets/sounds/` alongside the tgstation one-shots, so they are listed with bare filenames below.
...
| `stability_notice.ogg` | `Resources/Audio/Misc/notice2.ogg` | CC BY-SA 3.0 | Explicit `Misc/attributions.yml` | "Taken from TG station" — SS14 credits it upstream | [Pinned SS14 file](https://github.com/space-wizards/space-station-14/blob/1c7ebd24c5c36ac235c6122ebeedbc20bd44db3d/Resources/Audio/Misc/notice2.ogg) |
