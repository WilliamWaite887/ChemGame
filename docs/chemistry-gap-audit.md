...
| 5. Toxins and narcotics | Adapted | The narcotics table, controlled-drug chains, overdose, addiction, withdrawal, purge, contraband, evidence hooks, NPC mood behavior, and specialist counteragents are implemented. The toxin branch now covers isolated-report suppression, injury-conditional harm, accumulated terminal harm, data-driven delayed harm, nonlethal fatigue, multi-threshold escalation, delayed paralysis, and specific antidotes. The remaining gap is the latter specialist toxin roster and hydroponics-dependent herbicides. |
...
| 7. Delivery forms and tools | Adapted | Bottles, pills, syringes, patches, sprays, smoke, splash/puddles, charges, pH paper, grinder, analyzer/HPLC, and staged mixing exist. A dedicated smoke machine, portable mixer, chemical foam payload, and full route-specific metabolism remain gaps. |
| 8. Integration and completion | Unreviewed | Progression tiers, exact purity requests, frontier discovery, and save/network migration exist, but the final dependency rebalance and simulated full-save completion gate must follow the remaining content waves. |
...
- LAN and Steam compatibility fingerprints include reagent, reaction,
  produce, and status schemas, so indexed IDs cannot be interpreted against a
  different catalog.
...
| Delivery/tools | Core containers, patches, sprays, smoke, mixture-carrying foam, splash/puddles, charges, grinder, chamber, analyzer/HPLC, pH paper | Dedicated smoke machine, portable mixer, stronger route distinctions | `Adapted` |
